...
from apsrm.ext.office import (EgressGenerator, LunchGenerator,
...
def workplace(n_workers=40,
    ...
    meeting_room = Box(75.,
                       ...
                       name='meeting')
    open_plan = Box(open_plan_total_space,
                    BOX_TYPE.OPEN_PLAN,
                    None,
                    name='open plan')
    ...
    workplace.add_generator(
        MeetingGenerator(GATHERING_TYPE.MEETING, [WORKER_TYPE.WORKER],
                         [BOX_TYPE.MEETING]))
    ...
    generators = [
        lunch_generator, toilet_break_generator, working_generator,
        egress_generator
    ]
    ...
    for i in range(n_receptionists):
        configure_person(
            Person(48, role=WORKER_TYPE.RECEPTIONIST, work_box=foyer))
    for i in range(n_workers):
        configure_person(
            Person(48, role=WORKER_TYPE.WORKER, work_box=open_plan))
