...
from apsrm.ext.simulation import EmissionsCalculator, create_pathogen
...
def create_activity_weights(workplace):
    ...
    return {
        # for workers
        (worker_type.WORKER, box_type.OFFICE):
        'breathing',
        (worker_type.WORKER, box_type.OPEN_PLAN):
        'breathing',
        (worker_type.WORKER, box_type.TOILET):
        'breathing',
        (worker_type.WORKER, box_type.FOYER):
        'breathing_nose_mouth',
        (worker_type.WORKER, box_type.MEETING):
        lambda person, gathering: (
            ('speech_intermediate', 1. / len(gathering)),
            ('breathing', 1. - 1. / len(gathering))),
        (worker_type.WORKER, box_type.KITCHEN):
        (('speech_intermediate', .2), ('breathing', .8)),

        # for receptionists
        (worker_type.RECEPTIONIST, box_type.OPEN_PLAN):
        'breathing',
        (worker_type.RECEPTIONIST, box_type.TOILET):
        'breathing',
...
def test_smoke(workplace):
    ...
    external_ventilation[hvac.box_index] = sum([b.volume
                                                for b in boxes]) - hvac.volume

    pathogen = create_pathogen('delta')
    activity_weights, mask_wearing = create_activity_weights(workplace)

    emissions_calculator = EmissionsCalculator(pathogen, activity_weights,
                                               mask_wearing)
